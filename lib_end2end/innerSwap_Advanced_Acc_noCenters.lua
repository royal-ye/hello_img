...
-- DO NOT get self.mask in __init, just extract it from input, so that the InnerSwap layer
...
function InnerSwap:__init(cls, swap_sz, threshold, mask_thred)
    parent.__init(self)
...
function InnerSwap:updateOutput(input)
    ...
    self.c = self.c_real - 1  -- make sure that the mask_latent is only c == 1. Eg. 1*1*64*64

    -- use input 
    local former = input:narrow(2, 1, self.c/2):clone():squeeze() 
    local latter = input:narrow(2, self.c/2 + 1, self.c/2):clone():squeeze()

    local mask = input:narrow(2, self.c_real, 1):clone():squeeze()  -- 2D
...
function InnerSwap:updateGradInput(input, gradOutput)
    ...
    local mask_dim = torch.Tensor(1, 1, self.h, self.w):zero():typeAs(input)
    self.gradInput = torch.cat(self.gradInput, mask_dim, 2)
