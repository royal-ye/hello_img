...
opt = {
   ...
   display_plot = 'errG,errD,errL1',    -- which loss values to plot over time. Accepted values include a comma seperated list of: errL1, errG, and errD
   gpu = 1,                -- gpu = 0 is CPU mode. gpu=X is GPU mode on GPU X
   name = 'PLACE2_L1_L2',              -- name of the experiment, should generally be passed on the command line
   which_direction = 'AtoB',    -- AtoB or BtoA
   phase = 'train_place2',             -- train, val, test, etc
   preprocess = 'regular',      -- for special purpose preprocessing, e.g., for colorization, change this (selects preprocessing functions in util.lua)
   nThreads = 1,                -- # threads for loading data
   save_epoch_freq = 40,        -- save a model every save_epoch_freq epochs (does not overwrite previously saved models)
   ...
   checkpoints_dir = '/media/zsf/Software/model/model_en_de', -- models are saved here
...
idx_mask = {input_nc+1, input_nc+1}

if opt.display == 0 then opt.display = false end

opt.manualSeed = 6640--torch.random(1, 10000) -- fix seed
...
if opt.continue_train == 1 then
   ...
   netG = util.load(paths.concat(opt.checkpoints_dir, opt.name, '160_net_G.t7'), opt)
   print('loading previously trained netD...')
   netD = util.load(paths.concat(opt.checkpoints_dir, opt.name, '160_net_D.t7'), opt)
else
...
function createRealFake()
	...
	--image.save('mask_sq.png',mask_global[1]:mul(255))
	...
	temp_mask:copy(real_data[{ {}, idx_mask, {}, {} }]) 
	mask_global = 1 - temp_mask/255
	--image.save('mask_sq.png',mask_global[1]:mul(255))

	--[[print(mask_global:size())   
	print(temp_mask[{{},{1,10},{1,10}}])
	print(mask_global[{{},{1,10},{1,10}}])]]
	--assert(1==2)
	...
	--real_AB_local = real_B[{{},{},{left,left+127}, {top,top+127}}]:clone()
	--print(torch.type(real_AB_local))
    -- create fake
	...
	--fake_AB_local = fake_B[{{},{},{left,left+127}, {top,top+127}}]:clone()
	--print(torch.type(fake_AB_local))
end
...
local fGx = function(x)
    ...
    netG:backward(real_A, df_dg*2 + df_do_AE:mul(opt.lambda))
        ...
        if counter % opt.print_freq == 0 then
            ...
            print(('Epoch: [%d][%8d / %8d]\t Time: %.3f  DataTime: %.3f  '
                    .. '  Err_G: %.4f  Err_D: %.4f  ErrL1: %.4f'):format(
                     ...
                     errG, errD, errL1))
            print('sum gradParametersD:',torch.sum(gradParametersD))
			print('mean gradParametersD:',torch.mean(gradParametersD))
			print('sum gradParametersG:',torch.sum(gradParametersG))
			print('mean gradParametersG:',torch.mean(gradParametersG))
