...
function defineG_unet_128(input_nc, output_nc, ngf)
    ...
    local d1_ = e7 - nn.ReLU(true) - nn.SpatialFullConvolution(ngf * 8, ngf * 8, 4, 4, 2, 2, 1, 1) - nn.InstanceNormalization(ngf * 8) - nn.Dropout(0.5)
    -- input is (ngf * 8) x 2 x 2
    ...
    local d4_ = d3 - nn.ReLU(true) - nn.SpatialFullConvolution(ngf * 8 * 2, ngf * 4, 4, 4, 2, 2, 1, 1) - nn.InstanceNormalization(ngf * 4)
    -- input is (ngf * 8) x 16 x 16
    local d4 = {d4_,e3} - nn.JoinTable(2)
    local d5_ = d4 - nn.ReLU(true) - nn.SpatialFullConvolution(ngf * 4 * 2, ngf * 2, 4, 4, 2, 2, 1, 1) - nn.InstanceNormalization(ngf * 2)
    -- input is (ngf * 4) x 32 x 32
    local d5 = {d5_,e2} - nn.JoinTable(2)
    local d6_ = d5 - nn.ReLU(true) - nn.SpatialFullConvolution(ngf * 2 * 2, ngf, 4, 4, 2, 2, 1, 1) - nn.InstanceNormalization(ngf)
    -- input is (ngf * 2) x 64 x 64
...
function defineD_pixelGAN(input_nc, output_nc, ndf)
    ...
    netD:add(nn.InstanceNormalization(ndf * 2)):add(nn.LeakyReLU(0.2, true))
...
function defineD_n_layers(input_nc, output_nc, ndf, n_layers)
    if n_layers==0 then
        ...
        for n = 1, n_layers-1 do 
            ...
            netD:add(nn.InstanceNormalization(ndf * nf_mult)):add(nn.LeakyReLU(0.2, true))
        end
