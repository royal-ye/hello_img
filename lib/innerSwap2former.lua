...
-- here former mask region should be replaced with latter non-masked region. 
    ...
    local former_non_mask = former:clone()
    former_non_mask[ex_mask:byte()] = 0  --only save non_mask region
...
-- result_tmp should be an image that masked region swapped(with latter non-mask) and non-mask(garbage)
    ...
    local new_former = result_tmp_mask + former_non_mask
 
-- construct final self.output
    self.output = torch.cat(new_former, latter, 1):view(1, self.c, self.h, self.w)
    return self.output
end
...
function InnerSwap:updateGradInput(input, gradOutput)
    ...
    self.gradInput[{{},{self.c/2 + 1, self.c},{},{}}] = gradOutput[{{},{self.c/2 + 1, self.c},{},{}}]
    for cnt = 1, self.centers_lt:size(1) do
        ...
        self.gradInput[{{},{1, self.c/2},{lt_C[1],rb_C[1]},{lt_C[2],rb_C[2]}}] = 
