...
function InnerSwap:updateOutput(input)
	...
	--local mask_ = image.load('mask.png',1,'byte')
	--mask_ = mask_ / 255
    --self.mask = mask_ --mask:byte()测试时候改

	--print(self.mask:size())
	self.mask=self.mask:squeeze()
	--print(self.mask:size())
	--image.save('selfmask.png',self.mask*255)
    --self.threshold=5/16
    --print(self.threshold,'2')
	--print(self.mask)
    local mask = cal_feat_mask(self.mask:cuda(), self.conv_layers, self.threshold)  -- Badly need update
    --image.save('mask_8_16.png',mask:float():mul(255))
...
-- here former mask region should be replaced with latter non-masked region. 
    ...
    -- Then using the latter--U-net former part as content to construct swap(only use the dec part)
...
-- result_tmp should be an image that masked region swapped(with latter non-mask) and non-mask(garbage)
    local result_tmp = self.conv_new_dec:forward(self.kbar)  --改动
...
-- construct final self.output
    self.output = torch.cat(former, new_latter, 1):view(1, self.c, self.h, self.w)--new_latter
    return self.output
end
...
-- calculate the grad of PM using the grad of its replacing region in LN.

function InnerSwap:updateGradInput(input, gradOutput)
    self.gradInput = torch.Tensor():zero():typeAs(input):resizeAs(input)--FC前的梯度不变，更新FC后的梯度，自己的做法
    -- The latter isn't swapped, so just backprop the gradient.
    self.gradInput[{{},{1, self.c/2},{},{}}] = gradOutput[{{},{1, self.c/2},{},{}}]
    for cnt = 1, self.centers_lt:size(1) do
        ...
        self.gradInput[{{},{self.c/2 + 1, self.c},{lt_C[1],rb_C[1]},{lt_C[2],rb_C[2]}}] = 
