...
function InnerSwap:updateOutput(input)
    ...
    self.conv_enc, self.conv_dec, self.centers_lt, self.centers_rb, self.flag = NonparametricPatchAutoencoderFactory.buildAutoencoder(
                                                                    former, mask, self.swap_sz, 1, self.mask_thred, false, false, true)
    self.maxcoor = nn.MaxCoord():cuda()
    self.conv_enc:cuda()

    local tmp1 = self.conv_enc:forward(former)
    --although that patches are not all(mask patches excluded)
    -- yet self.ind log all patches most-similar pairs(the only difference is that only watch the non-masked region patcehs)
    self.kbar, self.ind = self.maxcoor:forward(tmp1)  
	--print(self.ind)
    -- Then using the latter as content to construct swap(only use the dec part)
    -- Use the all-patches, as the conv_new_dec should content 196.
...
-- calulate the real self.kbar and the  real self.ind
    ...
    local new_latter = self.conv_new_dec:forward(self.kbar)
...
-- This version is mainly for teacher's idea and 
-- Only work properly when batch_size is 1
-- Only update the latter part using gradient replacing, the former part do not change.
