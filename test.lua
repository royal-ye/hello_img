...
opt = {
    ...
    how_many = 100,         -- how many test images to run (set to all to run on every image found in the data/phase folder)
    which_direction = 'AtoB', -- AtoB or BtoA
    phase = 'test_place2',            -- train, val, test ,etc
    preprocess = 'regular',   -- for special purpose preprocessing, e.g., for colorization, change this (selects preprocessing functions in util.lua)
    aspect_ratio = 1.0,       -- aspect ratio of result images
    name = 'en_de_l1_BCE_01',                -- name of experiment, selects which model to run, should generally should be passed on command line
    ...
    which_epoch = '480',            -- which epoch to test? set to 'latest' to use latest cached model
}
...
opt.manualSeed = 100--torch.random(1, 10000) -- set seed
...
local netG = util.load('/media/zsf/Software/model/model_en_de/L2_L1_1W/480_net_G.t7', opt)
--paths.concat(opt.checkpoints_dir, opt.netG_name .. '.t7'

netG:evaluate()

printNet(netG)
--[[print(netG.modules[6])

print(netG.modules[6].weight[{{128,129},{10,11},{},{}}])

print(netG.modules[44])

print(netG.modules[44].weight[{{10,11},{1,2},{},{}}])
assert(1==2)]]
...
--[[
local mask = image.load('/home/zsf/pix2pix/mask.png', 1, 'byte')
local mask_temp = mask/255
mask_temp:byte()]]
--torch.repeatTensor(mask_global,mask_temp,opt.batchSize,1,1)
...
for n=1,math.floor(opt.how_many/opt.batchSize) do
	...
	temp_mask:copy(data_curr[{ {}, idx_mask, {}, {} }]) 
    ...
    local image_dir = paths.concat('results',opt.netG_name, 'images')
...
-- make webpage
io.output(paths.concat('results', opt.netG_name, 'index.html'))
